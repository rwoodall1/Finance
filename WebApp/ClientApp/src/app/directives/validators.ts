...
import { NG_VALIDATORS, AbstractControl, ValidatorFn, Validator, UntypedFormControl } from '@angular/forms';
...
@Directive({
    selector: '[hasEmoji][formControlName],[hasEmoji][formControl],[hasEmoji][ngModel]',
    providers: [
        { provide: NG_VALIDATORS, useExisting: forwardRef(() => EmojiValidator), multi: true }
    ],
    standalone: false
})

export class EmojiValidator implements Validator {
  constructor(@Attribute('hasEmoji') public hasEmoji: boolean) { };


  validate(c: AbstractControl): { [key: string]: any } {
    // self value
    let v = c.value;
    const regTest = new RegExp(/(\u00a9|\u00ae|[\u2000-\u3300]|\ud83c[\ud000-\udfff]|\ud83d[\ud000-\udfff]|\ud83e[\ud000-\udfff])/gi)
    let result = regTest.test(v);
...
@Directive({
    selector: '[notNumber][formControlName],[notNumber][formControl],[notNumber][ngModel]',
    providers: [
...
export class NumberValidator implements Validator {
  constructor(@Attribute('notNumber') public notNumber: string) { };
...
@Directive({
    selector: '[isZero][formControlName],[isZero][formControl],[isZero][ngModel]',
    providers: [
        { provide: NG_VALIDATORS, useExisting: forwardRef(() => ZeroValidator), multi: true }
    ],
    standalone: false
})

export class ZeroValidator implements Validator {
  constructor(@Attribute('isZero') public isZero: boolean) { };
...
@Directive({
    selector: '[emptyAccount][formControlName],[emptyAccount][formControl],[emptyAccount][ngModel]',
    providers: [
...
export class AccountValidator implements Validator {
  constructor(@Attribute('emptyAccount') public emptyAccount: boolean) { };
